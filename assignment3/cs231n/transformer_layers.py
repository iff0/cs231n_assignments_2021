...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pe[0] += torch.arange(max_len).unsqueeze(-1)
        j_vec = torch.arange(embed_dim)
        j_vec[1::2] -= torch.tensor(1)
        pe[0] *= torch.pow(1e4, -j_vec / torch.tensor(embed_dim))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        query = torch.transpose(torch.reshape(query, (N, S, H, E // H)), 1, 2)
        # (N, H, S, E / H)
        key = torch.transpose(torch.reshape(key, (N, T, H, E // H)), 1, 2)
        # (N, H, T, E / H)
        value = torch.transpose(torch.reshape(value, (N, T, H, E // H)), 1, 2)
        # (N, H, T, E / H)
        output = torch.matmul(query, torch.transpose(key, 2, 3)) / torch.sqrt(torch.tensor(E / H))
        # (N, H, S, T)
        if attn_mask is not None:
            ...
            #print(attn_mask)
            #print(attn_mask == 0)
            output = torch.masked_fill(output, attn_mask == 0, float('-inf'))
        output = F.softmax(output, dim=-1)
        output = self.dropout(output)
        output = torch.matmul(output, value)
        # (N, H, S, E/H)
        output = torch.reshape(torch.transpose(output, 1, 2), (N, S, E))
