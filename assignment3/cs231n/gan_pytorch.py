...
def sample_noise(batch_size, dim, seed=None):
    ...
    return torch.rand(batch_size, dim) * torch.tensor(2.) - torch.tensor(1.)
...
def discriminator_loss(logits_real, logits_fake):
    ...
    loss += bce_loss(logits_fake, torch.zeros(N).type(dtype))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return loss
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = (torch.mean(torch.square(scores_real - 1)) + torch.mean(torch.square(scores_fake))) * 0.5
...
def ls_generator_loss(scores_fake):
    ...
    loss = torch.mean(torch.square(scores_fake - 1)) * 0.5
