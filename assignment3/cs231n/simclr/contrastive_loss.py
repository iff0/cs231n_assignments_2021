...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = torch.sum(z_i * z_j) / (torch.linalg.norm(z_i, ord=2) * torch.linalg.norm(z_j, 2))
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        s1 = torch.exp(torch.mm(out, z_k.view(-1, 1)) / out_norm / torch.linalg.norm(z_k) / tau).squeeze()
        total_loss -= torch.log(s1[k + N] / (torch.sum(s1) - s1[k]))
        s2 = torch.exp(torch.mm(out, z_k_N.view(-1, 1)) / out_norm / torch.linalg.norm(z_k_N) / tau).squeeze()
        total_loss -= torch.log(s2[k] / (torch.sum(s2) - s2[k + N]))
...
def compute_sim_matrix(out):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    out_norm = torch.linalg.norm(out, dim=-1, keepdim=True)
    sim_matrix = torch.mm(out, out.T) / out_norm / (out_norm.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = torch.cat([sim_pos_exp, sim_pos_exp], dim=0)
