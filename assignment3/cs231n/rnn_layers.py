...
def rnn_step_backward(dnext_h, cache):
    ...
    d_o = (1. - next_h * next_h) * dnext_h
    dx = np.dot(d_o, Wx.T)
    dWx = np.dot(x.T, d_o)
    dprev_h = np.dot(d_o, Wh.T)
    dWh = np.dot(prev_h.T, d_o)
    db = np.sum(d_o, axis=0)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h_t = h0
    for t in range(T):
        h_t, _cache = rnn_step_forward(x[:, t, :], h_t, Wx, Wh, b)
        h[:, t, :] = h_t
        cache.append(_cache)
    cache.append(D)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def rnn_backward(dh, cache: list):
    ...
    for t in reversed(range(T)):
        dx[:, t, :], dnext_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h=dnext_h + dh[:, t, :], cache=cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dnext_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # [N, 4H]
    z_i = sigmoid(activation[:, :H])
    z_f = sigmoid(activation[:, H: 2 * H])
    z_o = sigmoid(activation[:, 2 * H: 3 * H])
    z_g = np.tanh(activation[:, 3 * H:])
    # [N, H]
    next_c = z_f * prev_c + z_i * z_g
    # [N, H]
    next_c_tanh = np.tanh(next_c)
    next_h = z_o * next_c_tanh
    cache = (z_o, next_c_tanh, z_f, prev_c, z_i, z_g, x, Wx, prev_h, Wh)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    z_o, next_c_tanh, z_f, prev_c, z_i, z_g, x, Wx, prev_h, Wh = cache
    dz_o = dnext_h * next_c_tanh
    dnext_c += dnext_h * z_o * (1 - next_c_tanh * next_c_tanh)

    dprev_c = dnext_c * z_f
    dz_f = dnext_c * prev_c
    dz_i = dnext_c * z_g
    dz_g = dnext_c * z_i
    dactivation = np.hstack((dz_i * z_i * (1 - z_i), dz_f * z_f * (1 - z_f), dz_o * z_o * (1 - z_o), (1 - z_g * z_g) * dz_g))
    #[N, 4H]
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    N, T, D = x.shape
    _, H = h0.shape
    h, caches = np.zeros((N, T, H)), []
    prev_h = h0
    prev_c = np.zeros_like(prev_h)
    for t in range(T):
        prev_h, prev_c, cache = lstm_step_forward(x=x[:, t, :], prev_h=prev_h, prev_c=prev_c, Wx=Wx, Wh=Wh, b=b)
        h[:, t, :] = prev_h
        caches.append(cache)
    caches.append(D)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def lstm_backward(dh, cache: list):
    ...
    for t in reversed(range(T)):
        dx[:, t, :], dprev_h, dprev_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h=dprev_h + dh[:, t, :], dnext_c=dprev_c, cache=cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dprev_h
